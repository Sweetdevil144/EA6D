...
import time  # 导入time模块以记录时间
# from metrics import warmup_lr  # not in repo, unused
...
def main(args):
    ...
    model_pose_decoder = pose_decoder().to(device)
...
#        lr_lambda = lambda iter: max((iter / train_num_steps) ** 0.96, train_lr)
#        self.lr_scheduler = torch.optim.lr_scheduler.LambdaLR(self.opt, lr_lambda=lr_lambda)
        
        #训练RA_encoder 8.16添加
        #self.opt = torch.optim.AdamW(model.parameters(), lr=cfg.trainer.lr)
        #self.lr_scheduler = warmup_lr.CosineAnnealingWarmupRestarts(self.opt, train_num_steps, max_lr=cfg.trainer.lr, min_lr=cfg.trainer.min_lr)
  ...
  #/home/data/chenhongyao/cube_data/train_output/model-118.pt
    def load(self, milestone):
        ...
        model.load_state_dict(data['model'], strict=False)  # 加上 strict=False
        ...
        if self.accelerator.is_main_process:
            ...
            self.ema.load_state_dict(data['ema'], strict=False)  # 加上 strict=False
    ...
    def save(self, milestone):
        ...
        if self.enable_resume:
            ...
            #torch.save(data, str('/root/autodl-tmp/models/duck/' f'model-{milestone}.pt'))
        else:
            data = {
                'model': self.accelerator.get_state_dict(self.model),
            }
    ...
    def train(self):
        ...
        # total_time = 0  # 初始化总时间
        # batch_count = 0  # 初始化批次数

        with tqdm(initial=self.step, total=self.train_num_steps, disable=not accelerator.is_main_process) as pbar:

            while self.step < self.train_num_steps:
                total_loss = 0.
                #total_loss_dict = {'loss_simple': 0., 'loss_vlb': 0., 'total_loss': 0., 'lr': 5e-5} #8.6 注释，原代码
                total_loss_dict = {'loss_simple': 0., 'loss_vlb': 0., 'loss_quaternion': 0.,'total_loss': 0., 'lr': 5e-5} #8.6 添加
                for ga_ind in range(self.gradient_accumulate_every):
                    ...
                    if self.step == 0 and ga_ind == 0:
                        if isinstance(self.model, nn.parallel.DistributedDataParallel):
                            ...
                            self.model.diffusion_extractor.on_train_batch_start(batch)
                    ...
                    with self.accelerator.autocast():
                        if isinstance(self.model, nn.parallel.DistributedDataParallel):
                            ...
                            loss, log_dict = self.model(batch)
                        ...
                        loss_quaternion = log_dict["train/loss_quaternion"].item() / self.gradient_accumulate_every #8.6 添加
                        total_loss_dict['loss_simple'] += loss_simple
                        total_loss_dict['loss_vlb'] += loss_vlb
                        total_loss_dict['loss_quaternion'] += loss_quaternion #8.6 添加
...
#                     with self.accelerator.autocast():
#                         if isinstance(self.model, nn.parallel.DistributedDataParallel):
#                             loss, log_dict, z = self.model.module.training_step(batch)
#                         else:
#                             loss, log_dict, z = self.model.training_step(batch)

#                         # 在这里计算新的损失函数，与 ground truth 做运算
#                         gt = batch['gt']  #  ground truth 变量
#                         new_loss = F.mse_loss(z, gt)  # 需要更改损失函数

#                         loss = (loss + new_loss) / self.gradient_accumulate_every
#                         total_loss += loss.item()
...
#                     self.accelerator.backward(loss)
                ...
                if accelerator.is_main_process:
                    ...
                    self.writer.add_scalar('loss_quaternion', loss_quaternion, self.step)
                ...
                if accelerator.is_main_process:
                    ...
                    if self.step != 0 and self.step % self.save_and_sample_every == 0:
                        ...
                        with torch.no_grad():
                            ...
                            # # 保存输入图片
                            # input_images = batch['cond']
                            # input_images_resized = F.interpolate(input_images, size=(256, 256), mode='bilinear')
                            # nrow = 2 ** math.floor(math.log2(math.sqrt(batch['cond'].shape[0])))
                            # tv.utils.save_image(input_images_resized, str(self.results_folder / f'input-{milestone}.png'),
                            #                     nrow=nrow)

                            # 保存输入图片
                            input_images = batch['cond'][:1]  # 只选择第一张图像
                            input_images_resized = F.interpolate(input_images, size=(256, 256), mode='bilinear')
                            input_images_denormalized = self.denormalize(input_images_resized)  # 反归一化
                            tv.utils.save_image(input_images_denormalized, str(self.results_folder / f'input-{milestone}.png'))
                            ...
                            if isinstance(self.model, nn.parallel.DistributedDataParallel):
                                ...
                                all_images, *_ = self.model.diffusion_extractor.sample(batch_size=batch['cond'].shape[0],
                                                  cond=batch['cond'],
                                                  mask=batch['ori_mask'] if 'ori_mask' in batch else None)
